...
public class ProductoController {
    ...
    public ResponseEntity<ServiceResponse<List<ProductoResponse>>> listarProductos() {
        ...
        return ResponseEntity
                .status(response.isOk() ? HttpStatus.OK : HttpStatus.INTERNAL_SERVER_ERROR)
                .body(response);
    }
    ...
    public ResponseEntity<ServiceResponse<Integer>> registrarProducto(@Valid @RequestBody ProductoRequest productoRequest) {
        ...
        return ResponseEntity
                .status(response.isOk() ? HttpStatus.CREATED : HttpStatus.BAD_REQUEST)
                .body(response);
    }
    ...
    public ResponseEntity<ServiceResponse<ProductoResponse>> obtenerProductoPorId(@PathVariable int id) {
        ServiceResponse<ProductoResponse> response = productoService.obtenerProductoPorId(id);
        return ResponseEntity
                .status(response.isOk() ? HttpStatus.OK : HttpStatus.NOT_FOUND)
                .body(response);
    }
    ...
    public ResponseEntity<ServiceResponse<String>> cargarDatosDePrueba() {
        ...
        if (insertadosCorrectamente > 0) {
            return ResponseEntity
                    ...
                    .body(ServiceResponse.exito("Se insertaron con éxito " + insertadosCorrectamente + " productos de prueba.", null));
        } else {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ServiceResponse.error("No se pudo insertar ningún producto de prueba."));
        }
    }
}